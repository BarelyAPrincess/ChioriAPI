...
public class SQLBackend extends PermissionBackend
{
	...
	public void nodeCommit( Permission perm )
	{
		try
		{
			...
			{
				if ( select.rowCount() > 1 )
					PermissionManager.getLogger().warning( String.format( "We found more then one permission node with the namespace '%s', please fix this, or you might experience unexpected behavior.", perm.getNamespace() ) );
				...
				if ( perm.getType() == PermissionType.DEFAULT && db.table( "permissions" ).delete().where( "permission" ).matches( perm.getNamespace() ).limit( 1 ).execute().rowCount() < 0 )
					...
					PermissionManager.getLogger().warning( "The SQLBackend failed to remove the permission node '" + perm.getNamespace() + "' from the database." );
				else
				{
