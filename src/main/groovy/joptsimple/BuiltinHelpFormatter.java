 ...
 *
 * <p>
 * The options section has two columns: the left column for the options, and the right column for their descriptions. The formatter will allow as much space as possible for the descriptions, by minimizing the option column's width, no greater than slightly
 * less than half the overall desired width.
 * </p>
 *
 * <p>
...
public class BuiltinHelpFormatter implements HelpFormatter
{
	 ...
	 *
	 * @param desiredOverallWidth
	 *             how many characters wide to make the overall help display
	 * @param desiredColumnSeparatorWidth
	 ...
	 * </p>
	 * <ul>
	 * <li>{@linkplain #findAndRemoveNonOptionsSpec(java.util.Collection) Finds and removes the non-option arguments descriptor}</li>
	 * <li>{@linkplain #shouldShowNonOptionArgumentDisplay(OptionDescriptor) Decides whether there is anything to show for non-option arguments}</li>
	 * <li>If there is, {@linkplain #addNonOptionRow(String) adds a header row} and {@linkplain #addNonOptionRow(String) adds a} {@linkplain #createNonOptionArgumentsDisplay(OptionDescriptor) non-option arguments description}</li>
	 * </ul>
	 *
	 * @param options
	 *             descriptors for the configured options of a parser
	 ...
	 *
	 * <p>
	 * This implementation loops over the given options, and for each, calls {@link #addOptionRow(String, String)} using the results of {@link #createOptionDisplay(OptionDescriptor)} and {@link #createDescriptionDisplay(OptionDescriptor)}, respectively,
	 * as arguments.
	 * </p>
	 *
	 * @param options
	 ...
	 * <li>If there are no options, calls {@link #addOptionRow(String)} with an argument that indicates that no options are specified.</li>
	 * <li>Otherwise, calls {@link #addHeaders(java.util.Collection)} with the options as the argument, followed by {@link #addOptions(java.util.Collection)} with the options as the argument.</li>
	 ...
	 * <li>if the description is present, then {@code ": "} plus the description if the type indicator is present; otherwise the description only</li>
	 * <li>{@code end}</li>
	 * </ul>
	 *
	 * @param buffer
	 ...
	 * <li>{@code ' '}</li>
	 * <li>{@code "default: "} plus the result of {@link #createDefaultValuesDisplay(java.util.List)}, surrounded by parentheses</li>
	 * </ul>
	 * </li>
	 * </ul>
	 *
	 ...
	 *
	 * <p>
	 * This implementation creates an empty string buffer and calls {@link #maybeAppendOptionInfo(StringBuilder, OptionDescriptor)} and {@link #maybeAppendNonOptionsDescription(StringBuilder, OptionDescriptor)}, passing them the buffer and the non-option
	 * arguments descriptor.
	 * </p>
	 *
	 * @param nonOptionDescriptor
	 *             non-option argument descriptor
	 ...
	 *
	 * <p>
	 * This implementation asks for the {@link OptionDescriptor#argumentTypeIndicator()} of the given descriptor, and if it is present and not {@code "java.lang.String"}, parses it as a fully qualified class name and returns the base name of that class;
	 ...
	 * <li>if either of the above is present, calls {@link #appendOptionHelp(StringBuilder, String, String, boolean)}</li>
	 * </ul>
	 *
	 * @param buffer
	 *             string buffer
 ...
 *
 * <p>
 * This implementation looks in the bundle {@code "joptsimple.HelpFormatterMessages"} in the default locale, using a key that is the concatenation of this class's fully qualified name, {@code '.'}, and the given key suffix, formats the corresponding
 * value using the given arguments, and returns the result.
 * </p>
 *
 * @param keySuffix
 *             suffix to use when looking up the bundle message
 * @param args
 *             arguments to fill in the message template with
...
protected String message( String keySuffix, Object... args )
{
	 ...
	 *
	 * <p>
	 * This implementation responds with {@code true} if the non-option descriptor has a non-{@code null}, non-empty value for any of {@link OptionDescriptor#description()}, {@link OptionDescriptor#argumentTypeIndicator()}, or
	 * {@link OptionDescriptor#argumentDescription()}.
	 * </p>
	 *
	 * @param nonOptionDescriptor
	 *             non-option argument descriptor
