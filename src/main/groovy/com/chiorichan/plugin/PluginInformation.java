...
public class PluginInformation
{
	...
	public List<MavenReference> getLibraries()
	{
		...
		for ( String mavenString : yaml.getStringList( "libraries" ) )
			...
			{
				PluginManager.getLogger().severe( "Could not parse the library '" + mavenString + "' for plugin '" + getName() + "', expected pattern 'group:name:version' unless fixed, it will be ignored." );
				PluginManager.getLogger().severe( e.getMessage() );
			}
