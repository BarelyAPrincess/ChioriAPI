 ...
 * <p>
 * IMPORTANT: A callback is only valid when invoked on the thread which created it. The results are undefined (and typically bad) when a callback is passed out to the operating system (or other code) in such a way that the callback is called from a
 * different thread.
 */
...
public class Callback
{
	 ...
	 * level should cause the messages described by the matching <code>Callback</code> objects to be invoked. This method is used to
	 ...
	 * should cause the messages described by the matching <code>Callback</code> objects to be invoked. This method is used to safely shut down SWT when
	...
	/**
	 * Immediately wipes out all native level state associated with <em>all</em> callbacks.
	 * <p>
	 * <b>WARNING:</b> This operation is <em>extremely</em> dangerous, and should never be performed by application code.
	 * </p>
	 */
